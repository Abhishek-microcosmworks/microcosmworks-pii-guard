...
import type {
  PIIEntity, PIIContext, RedactResult, RestoreResult, RestoreAndGuardResult,
  ResolvedConfig, FileRedactOptions, FileDetectOptions, FileRedactResult,
  FileDetectResult, TypeOverrideConfig, RedactStrategy,
} from './types.js';
...
export class PIIGuard {
  ...
  private typeOverrides: Record<string, TypeOverrideConfig>;
  ...
  async redact(text: string, opts: { scopeId: string }): Promise<RedactResult> {
    ...
    const entities: PIIEntity[] = detected
      .filter(d => {
    ...
    for (const entity of sorted) {
      ...
      if (strategy === 'synthetic' && entity.type === PIIType.EMAIL && entity.context.relatedEntities?.length) {
        ...
        if (nameEntity && nameEntity.synthetic) {
          const coherentEmail = this.syntheticGenerator.generateEmail(
            nameEntity.synthetic,
            entity.context,
            undefined
          );
  ...
  async restore(text: string, opts: { scopeId: string }): Promise<RestoreResult> {
    ...
    const sorted = [...mappingsWithTypes].sort(
  ...
  async restoreAndGuard(
    ...
    const mappingsWithTypes = await this.mappingManager.loadScopeWithTypes(opts.scopeId);
    const restored: { synthetic: string; original: string }[] = [];
    ...
    const rawNewEntities: PIIEntity[] = detected
      .filter(d => {
        const override = this.typeOverrides[d.type];
        if (override?.enabled === false) return false;
        return !exclusions.some(ex =>
          this.rangesOverlap(d.startIndex, d.endIndex, ex.start, ex.end),
        );
      })
      .map(d => {
        const override = this.typeOverrides[d.type];
   ...
   */
  private deduplicateEntities<T extends { startIndex: number; endIndex: number; confidence: number }>(
    entities: T[],
  ): T[] {
  ...
  async redactForEmbedding(text: string, opts: { scopeId: string }): Promise<RedactResult> {
  ...
  async redactFile(input: string | Buffer, opts: FileRedactOptions): Promise<FileRedactResult> {
    ...
    if (opts.outputPath && opts.format && !isWritableFormat(opts.format)) {
      throw new Error(
        `Cannot write output for format "${opts.format}". Only text-based formats support outputPath.`,
      );
    }
    ...
    if (opts.outputPath) {
      if (!isWritableFormat(extraction.format)) {
        throw new Error(
          `Cannot write output for format "${extraction.format}". Only text-based formats support outputPath.`,
        );
      }
  ...
  async detectFile(input: string | Buffer, opts?: FileDetectOptions): Promise<FileDetectResult> {
    const extraction = await extractText(input, {
      format: opts?.format,
      encoding: opts?.encoding,
    });
  ...
  async redactFileForEmbedding(input: string | Buffer, opts: FileRedactOptions): Promise<FileRedactResult> {
    return this.redactFile(input, opts);
  }

  private getStrategy(override?: TypeOverrideConfig): RedactStrategy | 'function' {
    if (!override?.strategy) return 'synthetic';
    if (typeof override.strategy === 'function') return 'function';
    return override.strategy;
  }

  private async resolveReplacement(entity: PIIEntity, scopeId: string): Promise<string> {
    const override = this.typeOverrides[entity.type];
    const strategy = this.getStrategy(override);

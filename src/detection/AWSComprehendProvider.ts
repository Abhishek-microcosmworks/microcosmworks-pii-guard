 ...
 */
const COMPREHEND_TYPE_MAP: Record<string, PIIType> = {
  NAME: PIIType.NAME,
 ...
 */
export class AWSComprehendProvider implements DetectionProvider {
  readonly name = 'aws-comprehend';

  private config: AWSComprehendConfig;
  private client: any; // ComprehendClient — typed as any to avoid hard dependency
  private DetectPiiEntitiesCommand: any;
  ...
  private async ensureClient(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        'pii-guard: @aws-sdk/client-comprehend is required for the aws-comprehend detection provider. ' +
  ...
  private splitAtSentenceBoundaries(text: string, maxBytes: number): string[] {
    const chunks: string[] = [];
    let remaining = text;

...
export class MappingManager {
  ...
  constructor(storage: StorageAdapter, cache: CacheAdapter | null, salt: string, cacheTtl: number = 3600) {
  ...
  async getOrCreate(scopeId: string, entity: PIIEntity, generator: SyntheticGenerator): Promise<string> {
  ...
  async resolveOriginal(scopeId: string, synthetic: string): Promise<string | null> {
  ...
  async loadScope(scopeId: string): Promise<Map<string, string>> {
  ...
  async loadScopeWithTypes(scopeId: string): Promise<Array<{
    synthetic: string;
    original: string;
    entityType: string;
  }>> {
    const records = await this.storage.findAllForScope(scopeId);
    const results: Array<{ synthetic: string; original: string; entityType: string }> = [];
  ...
  private hashEntity(value: string, scopeId: string): string {

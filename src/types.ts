...
export interface RestoreAndGuardResult {
  ...
  restored: { synthetic: string; original: string }[];
...
export interface StorageAdapter {
  findByHash(scopeId: string, entityHash: string): Promise<{ synthetic: string; entityType: string } | null>;
  ...
  findBySynthetic(scopeId: string, synthetic: string): Promise<{ entityHash: string; entityType: string; encryptedOriginal: string } | null>;
  findAllForScope(scopeId: string): Promise<Array<{ synthetic: string; entityType: string; encryptedOriginal: string }>>;
...
export interface FileRedactOptions {
  scopeId: string;
  outputPath?: string;     // write redacted text here (text formats only)
  format?: SupportedFileFormat;  // override auto-detection

...
export class HybridProvider {
    ...
    providers;
    strategy;
    constructor(providers, strategy = 'union') {
    ...
    async detect(text, options) {
        ...
        const results = await Promise.all(this.providers.map(provider => provider.detect(text, options)));
        // Flatten all entities
        const allEntities = results.flat();
        if (allEntities.length === 0)
            return [];
        // Sort by start index, then by confidence descending
        allEntities.sort((a, b) => a.startIndex - b.startIndex || b.confidence - a.confidence);
     ...
     */
    deduplicateUnion(entities) {
        const result = [];
        for (const entity of entities) {
            const overlapIdx = result.findIndex(existing => this.spansOverlap(existing, entity));
            if (overlapIdx === -1) {
     ...
     */
    deduplicateHighestConfidence(entities) {

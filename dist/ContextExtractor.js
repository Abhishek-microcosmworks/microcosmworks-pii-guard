...
export class ContextExtractor {
    ...
    buildRelationships(entities, fullText) {
        for (let i = 0; i < entities.length; i++) {
            ...
            if (entity.type === PIIType.EMAIL) {
                ...
                for (let j = 0; j < entities.length; j++) {
                    if (i === j)
                        continue;
                    if (entities[j].type !== PIIType.NAME)
                        continue;
                    const nameParts = entities[j].value.toLowerCase().split(/\s+/);
                    const nameMatchesEmail = nameParts.some(part => emailLocalPart.includes(part) && part.length > 2);
                    if (nameMatchesEmail) {
            ...
            if (entity.type === PIIType.EMAIL || entity.type === PIIType.PHONE) {
                const before = fullText.slice(Math.max(0, entity.startIndex - 100), entity.startIndex);

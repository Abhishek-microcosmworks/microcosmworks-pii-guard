...
function isDetectionProviderInstance(value) {
...
function resolveProviderByName(name, config, patterns, documentTypes) {
    if (name === 'builtin') {
...
export async function resolveConfig(config) {
    ...
    if (!storage) {
        if (dbType === 'mongodb') {
            ...
            try {
                ...
                if (err?.message?.includes('pii-guard:'))
                    throw err;
    ...
    if (!effectiveConfig?.awsComprehend) {
        ...
        if (awsRegion || needsAws) {
            if (!awsRegion) {
                throw new Error('pii-guard: PII_AWS_REGION env var (or awsComprehend.region config) is required ' +
            ...
            if (minConfidenceEnv !== undefined && minConfidenceEnv !== '') {
                ...
                if (isNaN(parsed) || parsed < 0 || parsed > 1) {
                    throw new Error('pii-guard: PII_AWS_MIN_CONFIDENCE must be a number between 0 and 1, ' +
                        `got "${minConfidenceEnv}"`);
                }
                minConfidence = parsed;
            }
            const awsComprehendFromEnv = {
                region: awsRegion,
    ...
    if (isDetectionProviderInstance(providerSetting)) {
        ...
        const subProviders = subProviderNames.map(p => {
            if (isDetectionProviderInstance(p))
                return p;
            return resolveProviderByName(p, effectiveConfig, patterns, documentTypes);
        });
        const strategy = effectiveConfig?.hybridDetection?.strategy || 'union';
        detectionProvider = new HybridProvider(subProviders, strategy);
    }
    else {
        detectionProvider = resolveProviderByName(providerSetting, effectiveConfig, patterns, documentTypes);
    }
    return {
        storage,
        cache,
...
function applyPatternOverrides(patterns, typeOverrides) {
